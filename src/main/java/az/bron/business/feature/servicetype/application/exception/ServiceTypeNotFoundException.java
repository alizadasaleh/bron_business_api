package az.bron.business.feature.servicetype.application.exception;

import az.gov.dlp.exception.NotFoundException;
import az.gov.dlp.exception.model.ErrorDetails;

public class ServiceTypeNotFoundException extends NotFoundException {
    public ServiceTypeNotFoundException() {
        super(ErrorDetails.of("", "ServiceType not found",
                "",
                ErrorDetails.error(314_00, "ServiceTypeNotFound")));
    }
}
